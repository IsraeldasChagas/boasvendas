...
@section('content')
    <div class="container" style="max-width: 32rem;">
        ...
        @else
            @php
                $abrirPainelCadastro = $errors->has('cadastro_telefone') || $errors->has('cadastro_cpf') || $errors->has('cadastro_email');
            @endphp

            <div class="vf-card p-0 mb-3 overflow-hidden">
                <button type="button"
                        class="btn btn-light w-100 text-start d-flex align-items-center justify-content-between gap-2 py-3 px-3 border-0 rounded-0 {{ $abrirPainelCadastro ? '' : 'collapsed' }}"
                        data-bs-toggle="collapse" data-bs-target="#collapseFidelidadeCadastro"
                        aria-expanded="{{ $abrirPainelCadastro ? 'true' : 'false' }}" aria-controls="collapseFidelidadeCadastro"
                        id="headingFidelidadeCadastro">
                    <span class="fw-semibold">Cadastrar ou atualizar meu cartão</span>
                    <i class="bi bi-chevron-down vf-fid-cadastro-chevron flex-shrink-0" aria-hidden="true"></i>
                </button>
                <div id="collapseFidelidadeCadastro" class="collapse {{ $abrirPainelCadastro ? 'show' : '' }}" aria-labelledby="headingFidelidadeCadastro">
                    <div class="px-3 pb-3 border-top bg-body-tertiary bg-opacity-50">
                        <p class="small text-muted mb-3 pt-3">Informe o telefone (WhatsApp), CPF e e-mail — use o <strong>mesmo telefone das suas compras</strong> na loja. Os selos são contados pela loja a cada compra.</p>
...
@push('styles')
    <style>
        #headingFidelidadeCadastro .vf-fid-cadastro-chevron {
            transition: transform 0.2s ease;
        }
        #headingFidelidadeCadastro:not(.collapsed) .vf-fid-cadastro-chevron {
