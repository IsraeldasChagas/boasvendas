...
@section('content')
    ...
    <h2 class="h5 fw-bold mb-3">Editar assinatura</h2>

    <div class="vf-card p-4" style="max-width: 40rem;">
        <form action="{{ route('admin.assinaturas.update', $assinatura) }}" method="post">
            @csrf
            @method('PUT')
            <div class="mb-3">
                <label class="form-label" for="empresa_nome">Nome da empresa</label>
                <input type="text" class="form-control @error('empresa_nome') is-invalid @enderror" id="empresa_nome" name="empresa_nome" value="{{ old('empresa_nome', $assinatura->empresa_nome) }}" required>
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('plano_id') is-invalid @enderror" id="plano_id" name="plano_id">
                    ...
                    @foreach ($planos as $plano)
                        <option value="{{ $plano->id }}" @selected(old('plano_id', $assinatura->plano_id) == $plano->id)>
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control @error('valor_mensal') is-invalid @enderror" id="valor_mensal" name="valor_mensal" value="{{ old('valor_mensal', $assinatura->valor_mensal) }}" min="0" step="0.01" required>
            ...
            <div class="mb-3">
                ...
                <input type="date" class="form-control @error('proxima_cobranca') is-invalid @enderror" id="proxima_cobranca" name="proxima_cobranca" value="{{ old('proxima_cobranca', $assinatura->proxima_cobranca->format('Y-m-d')) }}" required>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('gateway') is-invalid @enderror" id="gateway" name="gateway" value="{{ old('gateway', $assinatura->gateway) }}" placeholder="Ex.: Stripe, Asaas…">
