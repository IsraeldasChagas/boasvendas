{{--
  ...
  $opcoesPagasAberto: bool — mostra conteúdo e habilita envio
...
<div class="col-12">
    <button type="button"
        class="btn btn-outline-secondary btn-sm {{ $opcoesPagasAberto ? 'd-none' : '' }}"
        id="vf-btn-mostrar-opcoes-pagas"
        aria-expanded="{{ $opcoesPagasAberto ? 'true' : 'false' }}">
        <i class="bi bi-plus-circle me-1"></i>Adicionar opções pagas (acréscimos) ao produto
    </button>

    <div id="vf-opcoes-pagas-conteudo" class="{{ $opcoesPagasAberto ? '' : 'd-none' }}">
        <fieldset id="vf-opcoes-pagas-fieldset" class="border-0 p-0 m-0" @disabled(!$opcoesPagasAberto)>
            <legend class="float-none w-auto fs-6 fw-semibold mb-2">Opções pagas na loja</legend>
...
@once
    @push('scripts')
        <script>
            (function () {
                var btn = document.getElementById('vf-btn-mostrar-opcoes-pagas');
                var wrap = document.getElementById('vf-opcoes-pagas-conteudo');
                var fs = document.getElementById('vf-opcoes-pagas-fieldset');
                if (!btn || !wrap || !fs) return;
